...
if (window.innerWidth < 920) {
	...
	if (loginLink && loginImg) {
		loginLink.onclick = () => {
			loginImg.classList.add('mob_active');
		}
	}
	const headerCart = document.querySelector('.header__cart');
	const cartBalance = document.querySelector('.cart__balance');
...
loginImg.addEventListener('click', () => {
	loginImg.classList.add('_succes');
	loginForm.classList.add('_active');
	mainBlock.classList.add('_active');
	setTimeout(() => {
		loginImg.classList.remove('_succes');
	}, "500")
})

const loginBack = document.querySelector('.login__form__back');

loginBack.addEventListener('click', () => {
	loginBack.classList.add('_active');
	setTimeout(() => {
		loginBack.classList.remove('_active');
